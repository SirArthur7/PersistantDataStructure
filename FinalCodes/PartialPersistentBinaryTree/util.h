...
private:
    ...
    ////////////// NEEDED IN TREE, NOT IN LINKEDLIST
    /// to store where it is placed wrt its ancestor
    /// LEFT / RIGHT
    ///  -1   is it is at the starting
    int relativePosition_wrt_ancestor;
...
public:
    ...
    NODE(int _defaultVersion, int inDeg, int outDeg, int maxMod, int rel_Pos);
    NODE(int _defaultVersion, int _F1 , vector<NODE*>& _fwd_pointers, vector<NODE*>& _back_pointers, int inDeg, int outDeg, int maxMod, int rel_Pos);
    ...
    int getRealtivePosition_wrt_ancsestor();
...
template <class T_f1>
NODE<T_f1>::NODE(int _defaultVersion, int inDeg, int outDeg, int maxMod, int rel_Pos):cur_MOD_count(0),inDegree(inDeg),outDegree(outDeg),maxModCount(maxMod),relativePosition_wrt_ancestor(rel_Pos)
{
...
template <class T_f1>
NODE<T_f1>::NODE(int _defaultVersion, int _F1 , vector<NODE*>& _fwd_pointers, vector<NODE*>& _back_pointers, int inDeg, int outDeg, int maxMod, int rel_Pos):cur_MOD_count(0),inDegree(inDeg),outDegree(outDeg),maxModCount(maxMod),relativePosition_wrt_ancestor(rel_Pos)
{
    defaultVersion = _defaultVersion;
    F1 = _F1;
...
NODE<T_f1>* NODE<T_f1>::latestNonF1_Field_at(int index,int qr_ver){
    ...
    for(auto &x:modLogs){
        if(x->getMode()=='p')
            if((x->getVer()>=latest_ver) && (x->getVer()<=qr_ver) && (x->getVer()!=SENTINEL) && (x->getField_index()==index))
            {
                latest_ver =     x->getVer();
...
template <class T_f1>
int NODE<T_f1>::getRealtivePosition_wrt_ancsestor()
 {
     return relativePosition_wrt_ancestor;
 }
...
void NODE<T_f1>::printDetails(int ver)
{
    ...
    cout<<"Def Version: "<<getDefaultVersion()<<",   Relative Pos: "<<relativePosition_wrt_ancestor<<endl;
