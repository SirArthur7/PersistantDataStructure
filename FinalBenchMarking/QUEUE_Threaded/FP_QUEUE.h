...
Ancestry* create_thread(int n, Node *branch, Node *new_)
{
    Ancestry *tmp;
    int i;
    ...
    for(i=0; i<n; i++)
    {
        tmp[i].branch_pt=new_->thread[i].branch_pt;
        tmp[i].ancestor=new_->thread[i].ancestor;
    }
