...
// same thing
  for(auto _:state) {
    ...
    stack<int>s_e;
    consecutivePush(s_e,state.range(0));
    state.ResumeTiming();
      // timing resumes
      // turning off the compiler optimization
      //consecutivePop(s_p,state.range(0)); // 8.58 N
      //consecutivePop(s_e,state.range(0)); // 27.57 N
      randomizedPop(s_p,state.range(0)); //31.71 N 
